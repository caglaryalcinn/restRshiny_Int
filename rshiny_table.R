    ...
    tabPanel(
      ...
      mainPanel(
        tabsetPanel(type = "tabs",
                    ...
                    tabPanel("Univariate",varSelectInput("column_c", "categorik column seciniz:", catag),varSelectInput("column_n", "numeric column seciniz:", nume),plotOutput("plot"),
                             ),
                    tabPanel("Multivariate",verbatimTextOutput("plot2")),
...
server <- function(input, output, session){
  ...
  output$plot<- renderPlot({
    ggp3 <- ggplot(anket1, aes(x = !!input$column_n)) +    # Draw histogram & density
      geom_histogram(fill="#69b3a2",aes(y = ..density..),binwidth=100) + 
